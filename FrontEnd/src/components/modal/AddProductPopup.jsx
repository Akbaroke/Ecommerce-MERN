...
export default function AddProduct({ getCategory }) {
  ...
  const handleSubmit = e => {
    ...
    if (
      ...
      const sendDataStore = withRefreshToken(async () => {
        try {
          const { data } = await axiosMulti.post(`/api/product/${id}`, {
            nameProduct,
            price: priceProduct,
          ...
          setLoading(false)
          dispatch(
            alertShow('success', 'Add Product Successfully!', data.data.message)
          )
        } catch (err) {
          console.log(err)
          togleClosePopup()
  ...
  return (
    <Transition show={isOpen && type === 'CREATE_PRODUCT'}>
      <div
        tabIndex={-1}
        aria-hidden="true"
